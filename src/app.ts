...
// We include the file containing the namespace using the below syntax. followed by three forward slashes and reference tag with path attribute pointing to typescript file with namespace.
/// <reference path="./drag-drop-interface.ts" />
/// <reference path="./project-model.ts" />

//Even though we have included file containing namespace using reference tag. But still compiler will complains.
// We should also know that, varaibles in namespace can be accessed from the same namespace.
// So we can include all the executing code into the namespace containing the interfaces too.
...
//Now how to refactor the code into multipile file into modules
// We can divide the code into multiple files and includes respective script tags in head node to see how they work.
// for small projects this will work. but for medium size projects this will become cumbersome.

// We have two options to handle this in typescript
// We have Namespaces and Filebundling.

//Namespace is a special typescript syntax feature, where we group a part of code into a namespace and then import namespace into other file..
// We basically import this namespace into another file and access code from this namespace.
// typescript also allows bundle all these files into a single file. So we write code in multiple files which are then combined into a single file and they are imported from each other.

// An other alternative to using namespaces is, to use modern javascript ES6 import and export feature.
// we use import/export syntax from modern javascript.
// we will take the power of webpack to work in more performant way.